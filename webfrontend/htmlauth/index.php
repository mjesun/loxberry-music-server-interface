...
<?php
$html = LBLog::get_notifications_html( LBPPLUGINDIR, "settings");
echo $html;
?>
...
<?php
	...
	if ($return == 0) {
		echo " <span style='color:green; font-weight: bold;'>" . $L['SETTINGS.LABEL_RUNNING'] . "</span>";
	} else {
		echo " <span style='color:red; font-weight: bold;'>" . $L['SETTINGS.LABEL_NOTRUNNING'] . "</span>";
	}
?>
...
<div class="lb_flex-container">
	...
	<div class="lb_flex-item">
		...
		<script>$('input[id=music-servers<?= $cfg["data"]["music-servers"] ?>]').prop('checked', true);</script>
	</div>
	<div class="lb_flex-item-spacer"></div>
...
<div class="box">
	...
	<p>
		<b><font size="+2"><?=$L['SETTINGS.HINT_MUSICSERVERPORT_1']?> <?=$id?></b></font>
		<a href="http://<?=LBSystem::get_localip()?>:<?= 6090 + $id ?>/audio/cfg/all" class="ui-btn ui-btn-inline ui-mini" target="_blank"><?=$L['SETTINGS.BUTTON_TEST']?></a>
		<hr>
	</p>

	<div class="lb_flex-container">
		<div class="lb_flex-item-label">
		<label class="control-label"><?=$L['SETTINGS.LABEL_PORT']?></label>
		</div>
		<div class="lb_flex-item-spacer"></div>
		<div class="lb_flex-item">
			<b><?= 6090 + $id ?></b>
	...
	<div class="lb_flex-container">
		<div class="lb_flex-item-label">
		<label class="control-label"><?=$L['SETTINGS.LABEL_ZONES']?></label>
		</div>
		<div class="lb_flex-item-spacer"></div>
		<div class="lb_flex-item">
			<input type="range" min="1" max="30" name="music-server-<?= $id ?>-zones" value="<?= $cfg["data"]["music-server-$id-zones"] ?>" />
		</div>
		<div class="lb_flex-item-spacer"></div>
		<div class="lb_flex-item-help hint">
	...
	<div class="lb_flex-container">
		<div class="lb_flex-item-label">
		<label class="control-label"><?=$L['SETTINGS.LABEL_SENDTOMINISERVER']?></label>
		</div>
		<div class="lb_flex-item-spacer"></div>
		<div class="lb_flex-item">
	        <input type="hidden" name="music-server-<?= $id ?>-miniserver" value="0" />
	        <input type="checkbox" data-role="flipswitch"  name="music-server-<?= $id ?>-miniserver" value="1" <?= $cfg["data"]["music-server-$id-miniserver"] ? 'checked' : '' ?> />
		</div>
		<div class="lb_flex-item-spacer"></div>
		<div class="lb_flex-item-help hint">
	...
	<div class="lb_flex-container">
		<div class="lb_flex-item-label">
		<label class="control-label"><?=$L['SETTINGS.LABEL_IPTOSENDMESSAGES']?></label>
		</div>
		<div class="lb_flex-item-spacer"></div>
		<div class="lb_flex-item">
	...
	<div class="lb_flex-container">
		<div class="lb_flex-item-label">
		<label class="control-label"><?=$L['SETTINGS.LABEL_VIRTUALINOUT']?></label>
		</div>
		<div class="lb_flex-item-spacer"></div>
		<div class="lb_flex-item">
			<a class="ui-btn ui-input-btn ui-corner-all ui-shadow ui-icon-arrow-d ui-btn-icon-left ui-btn-inline ui-mini"
			href="data:application/octet-stream;charset=utf-8;base64,<?= base64_encode($vi) ?>"
			download="vi-msi-<?= 6090 + $id ?>.xml">
			<?=$L['SETTINGS.BUTTON_VIRTUALIN']?></a>
			<a class="ui-btn ui-input-btn ui-corner-all ui-shadow ui-icon-arrow-d ui-btn-icon-left ui-btn-inline ui-mini"
			href="data:application/octet-stream;charset=utf-8;base64,<?= base64_encode($vo) ?>"
			download="vo-msi-<?= 6090 + $id ?>.xml">
			<?=$L['SETTINGS.BUTTON_VIRTUALOUT']?></a>
		</div>
		<div class="lb_flex-item-spacer"></div>
		<div class="lb_flex-item-help hint">
